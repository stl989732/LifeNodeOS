...
export function readNativeGroceryList(): string[] {
  if (typeof window === "undefined") return [];
  try {
    const raw = window.localStorage.getItem(NATIVE_GROCERY_STORAGE_KEY);
    if (!raw) return [];
...
export function writeNativeGroceryList(items: string[]) {
  window.localStorage.setItem(NATIVE_GROCERY_STORAGE_KEY, JSON.stringify(items));
...
export function ensureNativeGrocerySeeded(defaults: string[]) {
  if (typeof window === "undefined") return;
  if (!window.localStorage.getItem(NATIVE_GROCERY_STORAGE_KEY)) {
    writeNativeGroceryList(defaults);
  }
}
